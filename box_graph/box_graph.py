...
try:
    ...
    from box_graph.point import ClusterPoint
...
class BoxGraph():
    ...
    def compute_box_neighbors(self, grid):
        ...
        sweepline = [[] for _ in range(len(grid))]
        while Q:
            (y, B, i) = Q.pop() # take last element (which has lowest priority)

            if y == B.bounds["bottom"]:
                for j in range(i-2, i+3): # recall, range is [i-2, i+3), so i+3 is not included
                    if 0 <= j and j < len(sweepline):
